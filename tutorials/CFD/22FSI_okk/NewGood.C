...
public:
    explicit reducedBasicFsi(tutorial22& FoamPb): problem(&FoamPb)
    {
        ...
        //problem->dyndict->subDict("sixDoFRigidBodyMotionCoeffs").set<scalar&>("mass", 0.11);
    ...
    void solveOnline_Pimple(scalar mu_now, int NmodesUproj, int NmodesPproj,fileName folder = "./ITHACAoutput/Online/")
    {
        ...
        //const_cast<pointVectorField&>(mesh.lookupObject<pointVectorField>("pointDisplacement"));
//ITHACAstream::writePoints(problem->GridPoints[0], folder, name(counter) + "/polyMesh/"); //???
        ...
        autoPtr<singlePhaseTransportModel> _laminarTransport = autoPtr<singlePhaseTransportModel>( new singlePhaseTransportModel(U, phi));
        singlePhaseTransportModel& laminarTransport = _laminarTransport();
        autoPtr<incompressible::turbulenceModel> turbulence = autoPtr<incompressible::turbulenceModel>
             (
        ...
        dimensionedScalar NewTime("NewTime", dimTime, 0.5);
        runTime.setTime(NewTime, 1); //Reset time
        //fileName path("./0.495");
 Info << "==============" << mesh.time().value() << "=============" << endl;
    //mkDir(path);
    //Foam::cp(./ITHACAoutput/Offline/0.45/U, path/U);
...
//#include "setInitialDeltaT.H"
        ...
        mesh.movePoints(problem->GridPoints[0]);   
        problem->_fvOptions = autoPtr<fv::options>(new fv::options(mesh));
        fv::options& fvOptions = problem->_fvOptions();
        problem->_pimple = autoPtr<pimpleControl>(new pimpleControl(mesh));
        pimpleControl& pimple = problem->_pimple();
        problem->_MRF = autoPtr<IOMRFZoneList>(new IOMRFZoneList(mesh));
        IOMRFZoneList& MRF = problem->_MRF(); 
        //sixDoFRigidBodyMotionSolver& sDRBMS = problem->sDRBMS(); //?????
        //sDRBMS.points0() =  mesh.points();
        point NewcentreOfMass(point(0.0, problem->centerofmassy[0], 0.0));
        vector Newvelocity(vector(0.0,    problem->vely[0], 0.0));
        problem->dyndict->subDict("sixDoFRigidBodyMotionCoeffs").set<point>("centreOfMass", NewcentreOfMass);
        problem->dyndict->subDict("sixDoFRigidBodyMotionCoeffs").set<vector>("velocity", Newvelocity);
        //mesh.movePoints(problem->GridPoints[0]); 
        /// Construct a new sixDoFRigidBodyMotionSolver
        autoPtr<sixDoFRigidBodyMotion> sDRBM = autoPtr<sixDoFRigidBodyMotion>(new sixDoFRigidBodyMotion(problem->dyndict->subDict("sixDoFRigidBodyMotionCoeffs"),
            problem->dyndict->subDict("sixDoFRigidBodyMotionCoeffs"), runTime ));
        sDRBM().status();
        autoPtr<sixDoFRigidBodyMotionSolver> sDRBMS = autoPtr<sixDoFRigidBodyMotionSolver>(new sixDoFRigidBodyMotionSolver(mesh, *(problem->dyndict)));
        
        //sDRBMS().points0() =  mesh.points() + problem->Dfield[0].primitiveFieldRef();
 
        std::cout << "==============" << sDRBMS().motion().mass() << "=============" << std::endl;
        
        //sDRBMS().pointDisplacement().primitiveFieldRef() =  problem->Dfield[0].primitiveFieldRef();
...
int main(int argc, char* argv[])
{
    ...
    int NmodesUout = para->ITHACAdict->lookupOrDefault<int>("NmodesUout", 2);
    int NmodesPout = para->ITHACAdict->lookupOrDefault<int>("NmodesPout", 2);
    ...
    example.finalTime  = 0.55;
    example.timeStep   = 1e-02;
    example.writeEvery = 1e-02;//5e-01;
    ...
    online.vely = example.vely;
    online.centerofmassy = example.centerofmassy;
    online.fomforcey = example.fomforcey;
    //(online.sixDofList).transfer(example.sixDofList);
   ...
   // online.dyndict->subDict("sixDoFRigidBodyMotionCoeffs").set("mass", 0.11);
    reducedBasicFsi reduced(online);

    reduced.startTime =  0.5;
