...
class NonLinTerm : public DEIM<volVectorField>
{
    public:
        using DEIM::DEIM;
...
#include "createFields.H" 
        ...
        void offlineSolve(fileName folder = "./ITHACAoutput/Offline") 
        {
            ...
            volVectorField& U = _U();
            //volVectorField& T = _T();
            volVectorField T("T",  -fvc::div(_phi(), _U()) - fvc::grad(_p()));
...
#include "createUfIfPresent.H"
        ...
        {
            dynamicFvMesh& mesh = meshPtr();
            DEIMObject = new NonLinTerm(Tfield, NmodesS, "convective_gradient", _T().name());
            DEIMObject->subfield = autoPtr<volVectorField>(new volVectorField(DEIMObject->generateSubmesh(0, mesh, _T())));
            // DEIMObject = new NonLinTerm(Ufield, NmodesS, "convective_gradient", _U().name());
            // DEIMObject->subfield = autoPtr<volVectorField>(new volVectorField(DEIMObject->generateSubmesh(0, mesh, _U())));
