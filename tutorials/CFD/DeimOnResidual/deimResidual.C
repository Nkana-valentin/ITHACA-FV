...
#include "createFields.H" 
        ...
        void offlineSolve(fileName folder = "./ITHACAoutput/Offline") 
        {
            ...
            //volVectorField S("S",  fvc::grad(_p())); /// For forcing term
...
struct newtonObject: public newton_argument<double>
{
    public:
        ...
        // Reduced source term coefficient
        Eigen::VectorXd s; //-fvc::grad(p) eval at deim point
};
...
int newtonObject::operator()(const Eigen::VectorXd& x, Eigen::VectorXd& fvec) const
{
    ...
    // Reduced Mass Term
    Eigen::VectorXd M1 = problem->Mr * a_dot;
    // Reduced Diffusion term
    Eigen::VectorXd M2 = problem->Br * x;
    // Reduced Convection term
    Eigen::VectorXd M3 = problem->Cr * s;
    // Reduced source term 
    Eigen::VectorXd M4 = problem->Hr; 
    //s = fvc::domainIntegrate(s).value();///?????????
    // Computation of the residual
    fvec = M1 +  M2 + M3 - M4; //last residual
        ...
        public:
           ...
           std::cout << "============================ inside reduced deim =================================" << std::endl;
                ...
                void onlineSolve(int NmodesU,  int NmodesP, int NmodesS, fileName folder = "./ITHACAoutput/Online") 
                {
                    ...
                    Eigen::VectorXd  thetaon = problem->DEIMObject->onlineCoeffs(T);
                        ...
                        volVectorField T2("Sonline", Foam2Eigen::Eigen2field(T, c)); //declare the field outside the loop
                    ...
                    while(runTime.run()){
                        ...
                        while (pimple.loop())
                        {
                            ...
                            (
                               fvm::ddt(U) 
                               //+ fvm::div(phi, U)
                               + problem->turbulence->divDevReff(U) //- fvm::laplacian(nueff, U)- fvc::div(nueff * dev2(T(fvc::grad(U))))
                               == T2 - fvc::grad(p) + fvOptions(U) 
                            );
                        ...
                        c = problem->DEIMObject->MatrixOnline*thetaon; //== newton_object.s
                        T2 =  Foam2Eigen::Eigen2field(T, c); //declare the field outside the loop
                        // problem->Umodes.reconstruct(URec, a, "URec");
...
//std::cout << "################ "<< fsideim.Tfield.size()<< " ##################" << std::endl;
    ...
    fsideim.PODDEIM(fsideim.NU, fsideim.NS);
