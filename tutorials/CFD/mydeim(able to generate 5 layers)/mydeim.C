...
#include "createUfIfPresent.H"
        ...
        void PODDEIM(int NmodesS)
        {
            ...
            DEIMObject->subfield = autoPtr<volVectorField>(new volVectorField(DEIMObject->generateSubmesh(5, mesh, _T())));
...
class ReducedFSIDEIM : public reductionProblem
{
        public:
           ...
           std::cout << "============================       inside reduced deim=================================" << std::endl;
                ...
                List<Eigen::VectorXd> listcoeffsU;
                List<Eigen::VectorXd> listcoeffsP;
                List<Eigen::VectorXd> listcoeffsC;
                volScalarModes Pmodes;
                volVectorModes Umodes;
                volVectorModes Tmodes;
                scalar startTime = 0.0;
                ...
                void onlineSolve(int NmodesU,  int NmodesP, int NmodesS, fileName folder = "./ITHACAoutput/Online") 
                {
                    ...
                    // Reconstruction of the initial solution
                        ...
                        while (pimple.loop())
                        {
                            ...
                            while (pimple.correct())
                            {
                                ...
                                while (pimple.correctNonOrthogonal())
                                {
                                    fvScalarMatrix pEqn
                                    (
                                        fvm::laplacian(rAtU(), p) == fvc::div(phiHbyA) // interpolate this term with eim (fvc::div(phiHbyA) volVectorField)
                                    );
...
std::cout << "################ "<< "End of the Offline part "<< " ##################" << std::endl;
    ...
    // vVf* DEIMObject = new vVf(fsideim.Tfield, fsideim.NmodesS, "convectiveterm", fsideim._T().name());
    // DEIMObject->subfield = autoPtr<volVectorField>(new volVectorField(DEIMObject->generateSubmesh(0, mesh, fsideim._T())));
...
//     return 0;
 ...
 // void liftSolve()
        ...
        // #include "UEqn.H"
        //         volScalarField rAU(1.0/UEqn1.A());
        //         volVectorField HbyA(constrainHbyA(rAU*UEqn1.H(), U, p));
        //         surfaceScalarField phiHbyA("phiHbyA", fvc::flux(HbyA) + fvc::interpolate(rAU)*fvc::ddtCorr(U, phi));
        ...
        //         Info << "Continuity error = " << mag(fvc::div(phi))().weightedAverage(mesh.V()).value() << endl;
