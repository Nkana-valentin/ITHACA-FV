...
#include "createFields.H" 
        ...
        autoPtr<DEIM_Pfield> DEIMfieldP;
...
#include "createUfIfPresent.H"
        ...
        void PODDEIM(int NmodesU, int NmodesP,int NmodesDEIMAU,int NmodesDEIMbU,int NmodesDEIMAP,int NmodesDEIMbP)
        {
            ...
            DEIMfieldU = autoPtr<DEIM_Ufield>(new DEIM_Ufield(Ufield, NmodesDEIMAU, "U", _U().name()));
            DEIMfieldP = autoPtr<DEIM_Pfield>(new DEIM_Pfield(Pfield, NmodesDEIMAP, "p", _p().name()));
            ...
            DEIMmatriceU->fieldsA = autoPtr<volVectorField>(new volVectorField(DEIMmatriceU->generateSubmeshMatrix(5, mesh, _U())));
            DEIMmatriceU->fieldsB = autoPtr<volVectorField>(new volVectorField(DEIMmatriceU->generateSubmeshVector(5, mesh, _U())));
            std::cout << "#####################################" << std::endl; 
            DEIMmatriceP->fieldsA = autoPtr<volScalarField>(new volScalarField(DEIMmatriceP->generateSubmeshMatrix(5, mesh, _p())));
            DEIMmatriceP->fieldsB = autoPtr<volScalarField>(new volScalarField(DEIMmatriceP->generateSubmeshVector(5, mesh, _p())));
            ...
            // // U Eqn
            //DEIMmatriceU->generateSubFieldMatrix(_p());
            //DEIMmatriceU->generateSubFieldMatrix(_U());
            ...
            {
//std::cout << DEIMmatriceU->MatrixOnlineA[i] << std::endl;
            ...
            {
//std::cout << DEIMmatriceP->MatrixOnlineA[i] << std::endl;
        ...
        public:
                ...
                void solveOnline(int NmodesUproj, int NmodesPproj, fileName folder = "./ITHACAoutput/Online/")
                {
                    ...
                    std::cout << "=================== "<< "line 703"  <<"===================" << std::endl;
                                    Eigen::MatrixXd thetaonAU = problem->onlineCoeffsAU(Ueqn1);
                                    Eigen::MatrixXd thetaonBU = problem->onlineCoeffsBU(Ueqn1);

                                    List<Eigen::MatrixXd> LinSysU = (problem->Umodes).project(Ueqn1, NmodesUproj);
                    ...
                    //                 for (int i = 0; i <(problem->ReducedVectorsB_U).size() ; ++i)
                    //                 {
                    // std::cout << problem->ReducedVectorsB_U[i] << std::endl;
                    // std::cout << "###############"  << "line 719" << "###############" << std::endl;

                    //                 }
                                    LinSysU[1] = EigenFunctions::MVproduct(problem->ReducedVectorsB_U, thetaonBU);
                                    //LinSysU[1] = problem->ReducedVectorsB_U[0]*  thetaonBU;
...
int main(int argc, char* argv[])
{
    ...
    fsideim.finalTime = 10;
    fsideim.timeStep = 0.01; //0.01;
    fsideim.writeEvery = 1;
