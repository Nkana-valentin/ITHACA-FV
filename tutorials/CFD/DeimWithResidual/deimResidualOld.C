        ...
        //#include "createUfIfPresent.H"
                    ...
                    while(runTime.run()){
                        ...
                        while (pimple.loop())
                        {
                            ...
                            //fvVectorMatrix UEqn2( -fvm::laplacian(nueff, U) -fvc::div(nueff * dev2(T(fvc::grad(U)))) );

                            fvVectorMatrix UEqn3( fvm::div(phi, U));
                            ...
                            // constexpr std::tuple<Eigen::MatrixXd, Eigen::VectorXd> projfvMat;
                            // constexpr std::tuple<Eigen::MatrixXd, Eigen::VectorXd> projfvMat1;
                            // constexpr std::tuple<Eigen::MatrixXd, Eigen::VectorXd> projfvMat2;
                            // constexpr std::tuple<Eigen::MatrixXd, Eigen::VectorXd> projfvMat3;
                            if (pimple.momentumPredictor())
                            {
                           
                                //projfvMat1 = Foam2Eigen::projectFvMatrix(UEqn1, problem->Umodes, NmodesU);
                                //Eigen::VectorXd ures1 = projfvMat1[0]*a0 -projfvMat1[1]; // should be a0_dot
                                //projfvMat2 = Foam2Eigen::projectFvMatrix(UEqn2, problem->Umodes, NmodesU);
                                //Eigen::VectorXd ures2 = projfvMat2[0]*a0 -projfvMat2[1];
                                //projfvMat3 = Foam2Eigen::projectFvMatrix(UEqn3, problem->Umodes, NmodesU);
                                //Eigen::VectorXd ures3 = projfvMat3[0]*a0 -projfvMat3[1];

                                //projfvMat = Foam2Eigen::projectFvMatrix(UEqn, problem->Umodes, NmodesU);
                                projfvMat = problem->Umodes.project(UEqn, NmodesU);

                                // Computation of the total residual
                                std::cout << projfvMat[0] << std::endl;
                                std::cout << "====================================" << std::endl;
                                std::cout << projfvMat[1] << std::endl;
                                //Eigen::VectorXd ures = projfvMat[0]*a0 - projfvMat[1];
                                std::cout << "====================================" << std::endl;
                                std::cout << ures << std::endl;
                                newton_object.R = projfvMat[0];
                                newton_object.r = projfvMat[1];
                                ...
                                hnls.solve(a); // ures  is the residual of the  non linear problem????????
                            ...
                            std::cout << "============================ line 680 =============================="<< std::endl;
                                ...
                                volVectorField U("U", problem->Umodes[0]); //?????
...
//std::cout << "################ "<< fsideim.Tfield.size()<< " ##################" << std::endl;
    //return 0;
