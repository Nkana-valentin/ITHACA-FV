...
#include "createUfIfPresent.H"
        ...
        void PODDEIM(int NmodesS)
        {
            ...
            //DEIMPField = new DeimScalarField(Pfield, NmodesS, "p", _p().name());
            //DEIMPField->subfield = autoPtr<volScalarField>(new volScalarField(DEIMPField->generateSubmesh(0, mesh, _p())));

            //std::cout << "#####################################" << std::endl; 
            //ModesUEig = Foam2Eigen::PtrList2Eigen(Umodes);
            //ModesUEig.conservativeResize(ModesUEig.rows(), NmodesU);
std::cout << "#####################################" << std::endl;
            //auto vol = ITHACAutilities::getMassMatrixFV(_T());
            //std::cout << vol.rows() << std::endl;
             //std::cout << vol.cols() << std::endl;
            //ReducedVectors = ModesUEig.transpose() * vol.asDiagonal()*DEIMObject->MatrixOnline;
                    ...
                    while(runTime.run()){
                        ...
                        while (pimple.loop())
                        {
                            ...
                            if (pimple.momentumPredictor())
                            {
                                ...
                                //UEqn1.solve();
                                //RedLinSysU = problem->Umodes.project(fvm::ddt(U) + problem->turbulence->divDevReff(U), NmodesU);
                                //Eigen::MatrixXd T2proj =  problem->Umodes.project(T2, NmodesU);
                                ...
                                //RedLinSysU[1] = RedLinSysU[1] + T2proj;
...
int main(int argc, char* argv[])
{
   ...
   //  PtrList<volVectorField> indexmodes; 
   //  label j=0;
   //  for (int i = 0; i <fsideim.Ufield.size() ; i++)//
   //  {
   //      //for (int j = 0; j < i; j = j+i.size()-1)
   //      //{
   //          //Info << j;
   //          indexmodes.insert(fsideim.Ufield[i][j]);
   //      //}
   //          j = j + fsideim.Ufield[0].size()-1;
   //  }

   // Info <<  indexmodes.size() << endl;
   // Info <<  indexmodes << endl;
   //indexmodes.toPtrList();
    // Info << fsideim.Ufield[0][0];
    //  Info << fsideim.Ufield[1][0];
    //   Info << fsideim.Ufield[2][0];
    ...
    test.startTime = 0.0;
    test.finalTime = 1;
    test.timeStep = 0.001; //0.01;
    test.writeEvery = 0.01;
    test.onlineSolve(fsideim.NmodesU, fsideim.NmodesP,  fsideim.NmodesS);
    //Info << ITHACAutilities::errorL2Rel(fsideim.Tfield, test.Tred);
