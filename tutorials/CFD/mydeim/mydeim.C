...
class ReducedFSIDEIM : public reductionProblem
{
        public:
                ...
                void onlineSolve(int NmodesU, int NmodesS, fileName folder = "./ITHACAoutput/Online") 
                {
                    ...
                    while(runTime.run()){
                        ...
                        while (pimple.loop())
                        {
                            ...
                            if (pimple.momentumPredictor())
                            {
                                ...
                                //UEqn.solve();
                                // RedLinSysU = Umodes.project(UEqn, NmodesU);
                                // Eigen::MatrixXd projT = Umodes.project(T, NmodesU);
                                // RedLinSysU[1] = RedLinSysU[1] + projT;
...
int main(int argc, char* argv[])
{
    ...
    fsideim.PODDEIM(fsideim.NmodesU, fsideim.NmodesS);
    // vVf test;
    // std::cout << "modes of the non linear part" << test.modes << std::endl;
    //Info << ITHACAutilities::errorL2Rel(fsideim.Tfield, fsideim.Sfield);
    //fsideim.DEIMObject->onlineCoeffs(fsideim.Tfield[0]);
    //std::cout << "**********************************************************************************" << std::endl;
    //fsideim.DEIMObject->onlineCoeffs(fsideim.Tfield[1]);
    ...
    test.onlineSolve(fsideim.NmodesU, fsideim.NmodesS);
    ...
    //Info << ITHACAutilities::errorL2Rel(fsideim.Ufield, test.Ured);
return 0;
