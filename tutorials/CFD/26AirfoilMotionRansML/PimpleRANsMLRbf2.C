    ...
    public:
        ...
        void getTurbNN()
        {
            if (!ITHACAutilities::check_folder("ITHACAoutput/NN/coeffs"))
            {
                ...
                //PtrList<pointVectorField> DisplacementData;
                ITHACAstream::read_fields(UfieldData, _U(), "./ITHACAoutput/Offline/");
                ITHACAstream::read_fields(PfieldData, _p(), "./ITHACAoutput/Offline/");
                //ITHACAstream::read_fields(DisplacementData, _pointDisplacement(), "./ITHACAoutput/Offline/");
                //Foam2Eigen::field2Eigen(DisplacementData);
                //auto nut_train = meshPtr().lookupObject<volScalarField>("nut");
                ...
                // std::cout << "Computing the coefficients for pointDisplacement" << std::endl;
                // Eigen::MatrixXd CoeffPointDisplacement_data = ITHACAutilities::getCoeffs(DisplacementData, Dmodes, 0, false);
                ...
                //cnpy::save(CoeffPointDisplacement_data, "ITHACAoutput/NN/coeffs/CoeffPointDisplacement_data.npy");
            }
        }
...
#include "createUfIfPresent.H"
            ...
            //nutCoeff = ITHACAutilities::getCoeffs(nut, nutModes, NmodesNut, true);
            // std::cout << nutCoeff.rows() << "\t" << nutCoeff.cols() << std::endl;
            // std::cout << nutCoeff << std::endl;
...
#include "CourantNo.H"
                ...
                //std::cout << nutCoeff.rows() << "\t" << nutCoeff.cols() << std::endl;
                // std::cout << nutCoeff << std::endl; 
                // nutCoeff = problem->evalNet(nutCoeff);
...
#include "test.H"
       ...
       //#include "correctPhi.H"
        ...
        #include "meshCourantNo.H"
                ...
                std::cout << nutCoeff.rows() << "\t" << nutCoeff.cols() << std::endl;
                std::cout << nutCoeff << std::endl; 
                /// Nut reconstruction using LSTM
                // Eigen::MatrixXd V = Foam2Eigen::PtrList2Eigen(nutModes); 
                // V.transposeInPlace();
                // Eigen::MatrixXd nutToEigen = Foam2Eigen::field2Eigen(nut); 
                // Eigen::MatrixXd c = V*nutToEigen;   
                //  // /// Eval the Lstm model                     
                // Eigen::MatrixXd nutCoeff = problem->evalLstm(c);
                // nutModes.reconstruct(nut, nutCoeff, "nut");
...
int main(int argc, char* argv[])
{
    ...
    example.startTime  = 0;
    example.finalTime  = 0.5; //0.05; //0.5;  
   ...
   // PtrList<pointVectorField> Dscale;
   // for (label i = 1; i < example.Dfield.size(); i++){
   //         //pointVectorField ydiff("pointDisplacement",  example.Dfield[i]- av);
   //         Dscale.append(example.Dfield[i].clone() );
   //  }
    ...
    //example.NNutModes = NmodesNutOut;
    ...
    if (!ITHACAutilities::check_folder("./ITHACAoutput/DataFromRom"))
    {
            ...
            ITHACAstream::exportMatrix(romforcex, "romforcex", "python","./ITHACAoutput/DataFromRom/");

            Eigen::VectorXd romforcey = Foam2Eigen::field2Eigen(reduced.romforcey);
            ITHACAstream::exportMatrix(romforcey, "romforcey", "python","./ITHACAoutput/DataFromRom/");

            Eigen::VectorXd Rx = Foam2Eigen::field2Eigen(reduced.Rx);
            ITHACAstream::exportMatrix(Rx, "Rx", "python","./ITHACAoutput/DataFromRom/");
            Eigen::VectorXd Ry = Foam2Eigen::field2Eigen(reduced.Ry);
            ITHACAstream::exportMatrix(Ry, "Ry", "python","./ITHACAoutput/DataFromRom/");

            Eigen::MatrixXd RedCentersOfMassy = Foam2Eigen::field2Eigen(reduced.centerofmassy);
            ITHACAstream::exportMatrix(RedCentersOfMassy, "RedCentersOfMassy", "python","./ITHACAoutput/DataFromRom/");
    }

    /// Export matrix for eddy viscosity coeffs
    ...
    ITHACAstream::exportMatrix(errL2Nut, "errL2Nut", "python","./ITHACAoutput/ErrorsL2/");
    //////////////////////////////////////////////////////////////////////////////////////////////
